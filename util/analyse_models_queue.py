...
# The machine learning models to predict the WAITING FOR CONSULTATION TIME
...
def generate_approach_v0(train, test):
	...
	waiting time of the training data
	...
	queue_duration_mean = train.waiting_4_consultation_duration.mean()
	y_pred = np.array( [queue_duration_mean for i in range(len(test.waiting_4_consultation_duration))] )
	return [np.sqrt(metrics.mean_squared_error(test.waiting_4_consultation_duration, y_pred))]


def generate_approach_v1(train, test):
	"""
	The baseline PLUS : return the RMSE considering the prediction as the average 
	waiting time of the training data, depending on the patient priority 
	(normal or urgent)
	...
	queue_duration_ifNormal_mean = train[train.priority == 0].waiting_4_consultation_duration.mean() 	
	queue_duration_ifUrgent_mean = train[train.priority == 1].waiting_4_consultation_duration.mean() 
	y_pred = np.array([ queue_duration_ifNormal_mean if i == 0 else 
		queue_duration_ifUrgent_mean for i in test.waiting_4_consultation_duration ])
	return [np.sqrt(metrics.mean_squared_error(test.waiting_4_consultation_duration.values, y_pred))]
...
def generate_approach_v2(train, test):
	...
	X_train = train.drop(['assessment_duration', 'waiting_4_assessment_duration', 
		...
		'consultation_start_time', 'day', 'patient', 'duration', 'waiting_4_consultation_duration'], axis=1)
	Y_train = train['waiting_4_consultation_duration']
	...
	for model in ml_models: 
		results.append(evaluate_model(model, ml_models_names[index], X_train, 
			Y_train, X_test, test.waiting_4_consultation_duration.values))
		index += 1
